...
#[tokio::test]
async fn test_set_prices_uses_dynamic_fee_from_simulation() {
    ...
    wiremock::Mock::given(wiremock::matchers::method("POST"))
        .respond_with(|req: &Request| {
            ...
            match method {
                ...
                "sendTransaction" => {
                    // Extract transaction and verify fee is derived dynamically (1500 + 100 base = 1600)
                    ...
                    match envelope {
                        TransactionEnvelope::Tx(v1_env) => {
                            assert_eq!(
                                v1_env.tx.fee, 1600,
                                "transaction fee must be dynamically derived from simulation minResourceFee (1500) + base (100)"
                            );
                        }
...
#[tokio::test]
async fn test_set_prices_falls_back_to_config_fee_when_simulation_fails() {
    let mock_server = MockServer::start().await;
    let rpc_url = mock_server.uri();
            ...
            match method {
                "simulateTransaction" => {
                    ResponseTemplate::new(200).set_body_json(serde_json::json!({
                        "jsonrpc": "2.0",
                        "id": 1,
                        "error": {"code": -32603, "message": "simulation error"}
                    }))
                }
                "getAccount" => {
                    ResponseTemplate::new(200).set_body_json(serde_json::json!({
                ...
                "sendTransaction" => {
                    ...
                    match envelope {
                        TransactionEnvelope::Tx(v1_env) => {
                            assert_eq!(
                                v1_env.tx.fee,
                                oracle::config::DEFAULT_SET_PRICES_TX_FEE,
                                "transaction fee must fall back to configured fee when simulation fails"
                            );
                        }
                        _ => panic!("expected TransactionEnvelope::Tx"),
                    }
    ...
    let result = oracle::keeper_loop::run_keeper_cycle(Arc::clone(&state)).await;
    assert!(result.is_ok(), "keeper cycle should succeed with fallback fee: {:?}", result.err());
}
