...
pub async fn oracle_status(
    ...
    let keeper_balance = match crate::keeper::check_keeper_balance(&keeper_cfg, &state.keeper_balance_below_min).await {
        Ok(stroops) => {
            let resp = crate::keeper::build_balance_response(&keeper_cfg, stroops);
            Some(resp.balance_xlm)
        }
        Err(err) => {
            tracing::warn!(error = %err, "failed to check keeper balance for oracle status");
            None
        }
    };
